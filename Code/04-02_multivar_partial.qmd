...
Perhaps multiple Xs (predictors) without a Y (outcome) - k means and PCA are examples of this
...
-   Dimensionality reduction - many variables, and not every variable is being used
    -   **Principal component analysis (PCA)**\
    -   Principal coordinate analysis (PCoA)\
    -   Factor analysis
-   Clustering
    -   **k-means**, c-means, x-means - distance based commonality analysis
...
-   Based on Y (outcome) existence:
    ...
    -   Supervised - explaining Y using x variables
    -   Unsupervised - crreating groups that are similar, without trying to explain Y

-   If Y exists (supervised), based on its type:

    -   Categorical: classification ex- high, medium and low yield; not an actual number but classes of Y
...
Is multi-collinearity the same as interaction effects in anova? What has happened with **mean_dayl.s_April** and **sum_prcp.mm_Oct** estimates and standard error when modeled i) alone, or with another uncorrelated variable vs. ii) with another correlated variable?
...
-   Dimensionality reduction
    ...
    -   By math - PCA (not the only one though)
-   Algorithm that handles multicollinearity
    -   Variable selection (LASSO)(Decision trees)\
    -   Multivariate by ensembling multiple bivariates (Random forest)
...
Length of the eigenvector signifies the size of variance. Let's run PCA for our weather data.
...
supervised v/s unsupervised - y variables are given in supervised classifcation: prediction of y variable using other x variable. 
...
k-means does not normalize our data for us like PCA did, so we will need to do that before running the model. If not done, weightage is given more to variables that have a large magnitude, irrespective of the units of the variables. 
...
```
Scale does a normalization leading to means being zero, with unit standard deviations. 
...
mod_km
```
nstart - no of times a full run must be done. 
...
```
elbow method - optimum clusters at wherever the curve bends (like an elbow)
...
```
average silhoutte width should be the highest 

total error: k=3 to 5 
silhouette: k=4
